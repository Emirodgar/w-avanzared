...
Vamos a utilizar las herramientas de Google Analytics y Google Tag Manager para poder registrar el estado de petición de cada página (correcto o con error).
...
- **404** error: indica que la página no existe (no ha podido ser encontrata)
...
Si no hiciéramos uso de Google Tag Manager, ya podríamos enviar la información directamente a Analytics añadiendo la siguiente instrucción en nuestro código fuente justo después del comando create pero antes del sendview. Ojo, hay que actualizar el número de dimensión, en este caso es dimension1 porque ha sido la primera en crearse pero si fuera la tercera tendríamos que utilizar dimension3.
...
Para que esto funcione debemos tener un template personalizado para cada estado de la página. En el template principal -del que se nutre todo el sitio, envíaríamos el valor 200. En el template de error página no encontrada, 404 y así con el resto que queramos medir.
...
- Introducimos el valor de la dimensión (que será el nombre de la variable que acabamos de crear en GTM que en este ejemplo es {{GA - httpStatus}}
